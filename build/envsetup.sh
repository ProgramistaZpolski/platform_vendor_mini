function __print_mini_functions_help() {
cat <<EOF
Additional MiniUI functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- minigerrit:   A Git wrapper that fetches/pushes patch from/to MiniUI Gerrit Review.
- minirebase:   Rebase a Gerrit change and push it again.
- miniremote:   Add git remote for MiniUI Gerrit Review.
- aospremote:      Add git remote for matching AOSP repository.
- cafremote:       Add git remote for matching CodeAurora repository.
- githubremote:    Add git remote for MiniUI Github.
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the Mini model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch mini_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/mini-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.mini.device | grep -q "$MINI_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $MINI_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function miniremote()
{
    ...
    local MINI_USER=$(git config --get review.review.minios.org.username)
    if [ -z "$MINI_USER" ]
    then
        git remote add mini ssh://review.minios.org:29418/$PFX$PROJECT
    else
        git remote add mini ssh://$MINI_USER@review.minios.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'mini' created"
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/MiniUI/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.mini.device | grep -q "$MINI_BUILD");
    then
...
function makerecipe() {
    ...
    then
        pwd
        miniremote
        git push mini HEAD:refs/heads/'$1'
    fi
    '
}
...
function minigerrit() {
    ...
    local user=`git config --get review.review.minios.org.username`
...
function minirebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "MiniUI Gerrit Rebase Usage: "
        echo "      minirebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.minios.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.mini.device | grep -q "$MINI_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/mini/build/tools/repopick.py $@
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $MINI_FIXUP_COMMON_OUT ]; then
