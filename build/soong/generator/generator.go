...
func init() {
	android.RegisterModuleType("mini_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = miniExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = miniExpandVariables(ctx, dep_file)
	...
	cmd := miniExpandVariables(ctx, String(g.properties.Cmd))
